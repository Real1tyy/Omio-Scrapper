...
export async function dismissGoogleSignIn(context: PlaywrightCrawlingContext): Promise<void> {
	...
	try {
		...
		const googleIframeHandle = await page.$(
		...
		if (googleIframeHandle) {
			...
			if (googleFrame) {
				...
				if (closeButton) {
					log.info(
						'Close button for Google sign-in overlay found inside iframe. Clicking it...',
					);
					...
					log.info(
						'Google sign-in overlay is present, but no close button was found inside the iframe.',
					);
				}
			} else {
